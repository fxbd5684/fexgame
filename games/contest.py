...
def 战斗主循环(enemyobj: User):
    ...
    temp_user_model: User = read_user_from_db()
    ...
    while True:
        ...
        if is_player_turn:
            ...
            伤害 = 计算对手受到的伤害(attack_value, temp_enemy_model.defense)
            input(f"你攻击了{temp_enemy_model.name}，造成了{伤害}点伤害")
            ...
            attack_value = 根据技能和基础攻击力_返回攻击值(skill_name, temp_user_model)
            伤害 = 计算对手受到的伤害(attack_value, temp_enemy_model.defense)
            input(f"{temp_enemy_model.name}攻击了你，造成了{伤害}点伤害")
